...
const HorarioEditable = () => {
    ...
    if (nuevoSector) {
      ...
      const nuevosSectoresData = sectoresData.map(sectorData => {
        if (sectorData.nombre === nuevoSector) {
          return {
            ...sectorData,
            agentes: [...sectorData.agentes, agenteData]
          };
        }
        if (sectorData.nombre === agenteData.sector) {
          return {
            ...sectorData,
            agentes: sectorData.agentes.filter(a => a.id !== agenteData.id)
          };
        }
        return sectorData;
      });
      ...
      // Verificar si el agente ya está asignado en el mismo horario (columna)
      const yaAsignadoEnHorario = nuevaMatriz.some((row, rowIndex) => 
        rowIndex !== fila && row[columna] && row[columna].includes(nombreCompleto)
      );

      if (yaAsignadoEnHorario) {
      ...
      if (nuevaMatriz[fila][columna]) {
        if (nuevaMatriz[fila][columna].includes(nombreCompleto)) {
          const match = nuevaMatriz[fila][columna].match(/\((\d+)\)/);
          const horas = match ? parseInt(match[1], 10) + 1 : 1;
          nuevaMatriz[fila][columna] = `${nombreCompleto} (${horas}h)`;
        } else {
          nuevaMatriz[fila][columna] = `${nombreCompleto} (1h)`;
        }
      } else {
        nuevaMatriz[fila][columna] = `${nombreCompleto} (1h)`;
      }
      ...
      // Actualizar las horas del agente
      const nuevosAgentes = agentes.map(agente => 
        agente.id === agenteData.id
          ? { ...agente, horas: calcularHorasAgente(nuevaMatriz, nombreCompleto) }
          : agente
      );
      setAgentes(nuevosAgentes);
    }
  ...
  const manejarClickFicha = (fila, columna) => {
    ...
    if (nuevaMatriz[fila][columna] !== null) {
      const match = nuevaMatriz[fila][columna].match(/(.+) \((\d+)h\)/);
      if (match) {
        const [, nombreCompleto, horasActuales] = match;
        if (parseInt(horasActuales) > 1) {
          nuevaMatriz[fila][columna] = `${nombreCompleto} (${parseInt(horasActuales) - 1}h)`;
        } else {
  ...
  const calcularHorasAgente = (matriz, nombreCompleto) => {
    return matriz.reduce((total, fila) => 
      total + fila.reduce((rowTotal, celda) => {
        if (celda && celda.includes(nombreCompleto)) {
          const match = celda.match(/\((\d+)h\)/);
          const horas = match ? parseInt(match[1], 10) : 0;
          return rowTotal + horas;  
        }
        return rowTotal;
      }, 0), 0
    );
